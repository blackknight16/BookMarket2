//-----------------------------------------------------------------------
// Стандартный контроллер, входящий в шаблон MVC4 Application, который
// аутентифицирует пользователя (регистрирует, запускает, восстанавливает
// пароль). Здесь я изменил только UserProfile, добавив к нему внешний
// ключ на Individual. А также редирект Admin на собственный лейаут.
//-----------------------------------------------------------------------
using System;
...
namespace BookMarket.Controllers
{
    ...
    {
        private IDbModelRepository _individualRepository = new IndividualRepository();
        private IDbModelRepository _userProfileRepository = new UserProfileRepository();
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                //Если входит администратор
                if (model.UserName.ToLower() == MarketDbInitializer.ADMIN_REGISTER_MODEL.UserName
                    && model.Password == MarketDbInitializer.ADMIN_REGISTER_MODEL.Password)
        ...
        public ActionResult Register(RegisterModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    userProfile = ((UserProfileRepository) this._userProfileRepository)
                        .FindByUserName(model.UserName);

                    individual = model.GetIndividual(); //извлекли перс.данные
                    if (individual.UserProfiles == null)
                        individual.UserProfiles = new List<UserProfile>();
                    individual.UserProfiles.Add(userProfile);

                    //Добавление Individual только через Service!
                    individualService.AddIndividual(individual, userProfile);
